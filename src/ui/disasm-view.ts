import { cpuMemoryMap, prgMapping, readCpu } from '../nes/cpu-map.ts';
...
export function createDisasmView(rom: NesRom, nav: Navigator): HTMLElement {
  ...
  const areas = cpuMemoryMap(rom, fixed);
  ...
  for (const e of vectors?.entries ?? []) {
    ...
    b.addEventListener('click', () => nav.go({ view: 'disasm', cpu: e.target!.cpu }));
    quick.append(b);
  }
  ...
  function operandCell(l: DisasmLine, title: string): HTMLElement {
    ...
    if (l.target !== null) {
      ...
      if (readCpu(rom, m, ref)) {
        link = nav.link({ view: 'disasm', cpu: ref }, text);
        link.title = 'クリックで飛び先を逆アセンブル';
      } else {
      ...
      link = nav.link({ view: 'cpu', cpu: ref }, text);
  ...
  function row(l: DisasmLine): HTMLTableRowElement[] {
    ...
    // CPU アドレス表示は固定の対応でしか byte を引けないため、bank 切り替えのある Mapper ではリンクにしない
    const cpuCell = fixed ? nav.link({ view: 'cpu', cpu: l.cpu }, addr(l.cpu)) : addr(l.cpu);
