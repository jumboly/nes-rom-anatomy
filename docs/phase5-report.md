...
- #22 同じ byte が続く範囲（未使用の `$FF` 埋めなど）を 1 行に畳んで表示する。線形逆アセンブルでは `ISC $FFFF,X` が延々と並ぶ
- #23 飛び先をたどる recursive traversal でコード / データを判定する（線形逆アセンブルの限界を補う）
- #24 ca65 の `.dbg` / map ファイルを読み込んでシンボル名をラベル・operand に表示する（nrom-template で検証できる）
- #25 命令ごとのサイクル数・変化するフラグの表示
- #20 のうち、「CPU アドレスから引く」から逆アセンブルへのリンクは Phase 6 で扱う（Issue にコメント済み）
