...
- #18 推定 Mapper で「全 bank の末尾 6 byte が同じか」を調べて表示する。MMC1 / AxROM のゲームは全 bank に同じベクタを置くことが多く、一致すれば推定の確度が上がる
- #19 Hex Viewer でベクタの 6 byte と飛び先を色やマークで示す
- #20 ベクタの飛び先から「CPU アドレスから引く」・逆アセンブル表示へのリンク（Phase 5 / 6 と合わせる）
- #21 電源投入時の状態を Mapper ごとに調べて `powerOnLastBank` を拡充する（現状は 1 / 2 / 4 / 7 以外は 8 KiB の仮定）
